...
export default function Finance() {
  ...
  const totalIncome = filteredRecords?.filter((r) => r.type === "income").reduce((sum, r) => sum + r.amount, 0) || 0;
  const totalOutcome = filteredRecords?.filter((r) => r.type === "outcome").reduce((sum, r) => sum + r.amount, 0) || 0;
  ...
  const exportToPDF = () => {
    ...
    // Summary
    doc.setFontSize(11);
    doc.text(`Total Pemasukan: Rp ${totalIncome.toLocaleString("id-ID")}`, 14, 52);
    doc.text(`Total Pengeluaran: Rp ${totalOutcome.toLocaleString("id-ID")}`, 14, 60);
    ...
    doc.save(`laporan-keuangan-${filterYear}-${filterMonth === "all" ? "tahunan" : filterMonth}.pdf`);
  ...
  const exportToExcel = () => {
    ...
    // Add summary rows
    data.push({ Tanggal: "", Jenis: "", Kategori: "", Deskripsi: "", Jumlah: 0 });
    data.push({ Tanggal: "", Jenis: "", Kategori: "", Deskripsi: "Total Pemasukan", Jumlah: totalIncome });
    data.push({ Tanggal: "", Jenis: "", Kategori: "", Deskripsi: "Total Pengeluaran", Jumlah: totalOutcome });
    data.push({ Tanggal: "", Jenis: "", Kategori: "", Deskripsi: "Saldo", Jumlah: balance });

    const ws = XLSX.utils.json_to_sheet(data);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Laporan Keuangan");
    XLSX.writeFile(wb, `laporan-keuangan-${filterYear}-${filterMonth === "all" ? "tahunan" : filterMonth}.xlsx`);
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-xl sm:text-2xl font-display font-bold">Laporan Keuangan</h1>
      ...
      <div className="grid gap-3 sm:gap-4 grid-cols-1 sm:grid-cols-3">
        ...
        <Card className={balance >= 0 ? "border-primary/20 bg-primary/5" : "border-orange-500/20 bg-orange-500/5"}>
          ...
          <CardContent>
            <div className={`text-lg sm:text-2xl font-bold ${balance >= 0 ? "text-primary" : "text-orange-600"}`}>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab}>
              <TabsList className="h-8">
                ...
                <TabsTrigger value="income" className="text-xs px-2 sm:px-3">Masuk</TabsTrigger>
                <TabsTrigger value="outcome" className="text-xs px-2 sm:px-3">Keluar</TabsTrigger>
              </TabsList>
            </Tabs>
          </div>
        </CardHeader>
                ...
                <TableBody>
                  {filteredRecords.map((record) => (
                    <TableRow key={record.id}>
                      ...
                      <TableCell className={`text-right text-xs sm:text-sm font-medium py-2 ${record.type === "income" ? "text-green-600" : "text-red-600"}`}>
