...
export default function Payments() {
  ...
  const filteredPayments = payments?.filter((p) => {
    const monthMatch = filterMonth === "all" || p.month.toString() === filterMonth;
  ...
  const generateReminderMessage = () => {
    const deadlineFormatted = format(new Date(reminderData.deadline), "dd MMMM yyyy", { locale: localeId });
...
Dengan hormat, kami ingatkan untuk pembayaran *Iuran Bulanan* periode *${MONTHS[reminderData.month - 1]} ${reminderData.year}*.
...
_Pesona Kenari Townhouse_`;
  ...
  const exportToPDF = () => {
    const doc = new jsPDF();
    const periodText = filterMonth === "all" 
      ? `Tahun ${filterYear}` 
    ...
    doc.text(`Tanggal Cetak: ${format(new Date(), "dd MMMM yyyy", { locale: localeId })}`, 14, 40);

    const totalPaid = filteredPayments?.filter(p => p.status === "paid").reduce((sum, p) => sum + p.amount, 0) || 0;
    const totalPending = filteredPayments?.filter(p => p.status === "pending").length || 0;
    ...
    doc.save(`laporan-iuran-${filterYear}-${filterMonth === "all" ? "tahunan" : filterMonth}.pdf`);
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(wb, `laporan-iuran-${filterYear}-${filterMonth === "all" ? "tahunan" : filterMonth}.xlsx`);
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-xl sm:text-2xl font-display font-bold">Pembayaran Iuran</h1>
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-2">
                        ...
                        <Select
                          ...
                          onValueChange={(v) =>
                            setReminderData({ ...reminderData, month: parseInt(v) })
                          }
                        >
                      ...
                      <div className="space-y-2">
                        ...
                        <Select
                          ...
                          onValueChange={(v) =>
                            setReminderData({ ...reminderData, year: parseInt(v) })
                          }
                        >
                          <SelectTrigger className="h-9">
                            <SelectValue />
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setReminderData({ ...reminderData, amount: e.target.value })
                        }
                        className="h-9"
                      />
                    </div>
                    ...
                    <div className="space-y-2">
                      <Label className="text-xs">Batas Waktu Pembayaran</Label>
                      <Input
                        type="date"
                        value={reminderData.deadline}
                        onChange={(e) =>
                          setReminderData({ ...reminderData, deadline: e.target.value })
                        }
                        className="h-9"
                      />
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setReminderData({ ...reminderData, bankAccount: e.target.value })
                        }
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base sm:text-lg">Daftar Pembayaran</CardTitle>
        </CardHeader>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="text-xs hidden sm:table-cell">Jumlah</TableHead>
                  ...
                  {filteredPayments.map((payment) => (
                    <TableRow key={payment.id}>
                      ...
                      <TableCell className="text-xs sm:text-sm py-2">
                        <span className="hidden sm:inline">{MONTHS[payment.month - 1]}</span>
                        <span className="sm:hidden">{MONTHS[payment.month - 1].slice(0, 3)}</span>
                        {" "}{payment.year}
                      </TableCell>
                      ...
                      <TableCell className="py-2">
                        <Badge className={`${STATUS_COLORS[payment.status]} text-xs`}>
                          <span className="hidden sm:inline">{STATUS_LABELS[payment.status]}</span>
                          <span className="sm:hidden">
                            {payment.status === "pending" ? "Pending" : payment.status === "paid" ? "Lunas" : "Belum"}
                          </span>
                        </Badge>
                      </TableCell>
          ...
          {selectedPayment && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div>
                  <span className="text-muted-foreground text-xs">Rumah:</span>{" "}
