...
import { ArrowLeft, Search, UserCog, Shield, Users, Loader2 } from 'lucide-react';
import { Link } from 'react-router-dom';
import type { AppRole, PengurusTitle, Profile, UserRole } from '@/types/database';
...
export default function AdminUsers() {
  ...
  const updateRoleMutation = useMutation({
    mutationFn: async ({ userId, role, title }: { userId: string; role: AppRole; title: PengurusTitle | null }) => {
  ...
  const getRoleBadge = (role: AppRole | undefined, title: PengurusTitle | null | undefined) => {
    if (role === 'admin') {
        ...
        <div className="grid grid-cols-3 gap-4">
          <Card>
            <CardContent className="p-4 flex items-center gap-4">
              ...
              <div>
                <p className="text-2xl font-bold">{users?.filter(u => u.user_role?.role === 'admin').length || 0}</p>
          ...
          <Card>
            <CardContent className="p-4 flex items-center gap-4">
              ...
              <div>
                <p className="text-2xl font-bold">{users?.filter(u => u.user_role?.role === 'pengurus').length || 0}</p>
          ...
          <Card>
            <CardContent className="p-4 flex items-center gap-4">
              ...
              <div>
                <p className="text-2xl font-bold">{users?.filter(u => u.user_role?.role === 'warga' || !u.user_role).length || 0}</p>
                  ...
                  <TableBody>
                    {filteredUsers?.map((user) => (
                      <TableRow key={user.id}>
                        ...
                        <TableCell>
                          {getRoleBadge(user.user_role?.role, user.user_role?.title)}
                        </TableCell>
                        <TableCell>
            ...
            <div className="space-y-4 py-4">
              ...
              {newRole === 'pengurus' && (
                <div className="space-y-2">
                  ...
                  <Select value={newTitle} onValueChange={(value) => setNewTitle(value as PengurusTitle)}>
                    <SelectTrigger>
                      <SelectValue placeholder="Pilih jabatan" />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="ketua">Ketua RT</SelectItem>
                      <SelectItem value="wakil_ketua">Wakil Ketua RT</SelectItem>
                      <SelectItem value="sekretaris">Sekretaris</SelectItem>
                      <SelectItem value="bendahara">Bendahara</SelectItem>
                      <SelectItem value="sie_keamanan">Sie. Keamanan</SelectItem>
                      <SelectItem value="sie_kebersihan">Sie. Kebersihan</SelectItem>
